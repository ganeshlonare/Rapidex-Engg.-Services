...
export async function middleware(request) {
    try {
        ...
        if ((pathname.startsWith('/my-account') || pathname.startsWith('/profile') || pathname.startsWith('/orders') || pathname.startsWith('/cart') || pathname.startsWith('/checkout')) && role !== 'user') {
            return NextResponse.redirect(new URL(WEBSITE_LOGIN, request.nextUrl))
        }
...
export const config = {
    matcher: ['/admin/:path*', '/my-account/:path*', '/auth/:path*', '/orders/:path*', '/cart/:path*', '/profile/:path*', '/checkout/:path*']
}