...
export async function PUT(request) {
    try {
        ...
        const data = await (ProductVariantModel as any).find({ _id: { $in: ids } }).lean()
        if (!data.length) {
        ...
        if (deleteType === 'SD') {
            await (ProductVariantModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await (ProductVariantModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } });
        }
...
export async function DELETE(request) {
    ...
    try {
        ...
        await (ProductVariantModel as any).deleteMany({ _id: { $in: ids } })
