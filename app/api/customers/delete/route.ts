...
export async function PUT(request) {
    try {
        ...
        const data = await (UserModel as any).find({ _id: { $in: ids } }).lean()
        if (!data.length) {
        ...
        if (deleteType === 'SD') {
            await (UserModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await (UserModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } });
        }
...
export async function DELETE(request) {
    ...
    try {
        ...
        await (UserModel as any).deleteMany({ _id: { $in: ids } })
