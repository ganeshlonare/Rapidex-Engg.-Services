...
export async function PUT(request) {
    try {
        ...
        const data = await (OrderModel as any).find({ _id: { $in: ids } }).lean()
        if (!data.length) {
        ...
        if (deleteType === 'SD') {
            await (OrderModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await (OrderModel as any).updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } });
        }
...
export async function DELETE(request) {
    ...
    try {
        ...
        await (OrderModel as any).deleteMany({ _id: { $in: ids } })
